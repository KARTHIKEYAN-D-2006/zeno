...
function openHome(email) {
  localStorage.setItem('zenoUserEmail', email);
...
signinForm.addEventListener('submit', async e => {
  ...
  try {
    ...
    openHome(cred.user.email || email);
  } catch (err) {
...
registerForm.addEventListener('submit', async e => {
  ...
  try {
    ...
    // Optionally you could save "name" to Firestore/profile; for now we just continue.
...
async function loginWithGithub() {
  ...
  const email = result.user.email || `${result.user.uid}@github`;
  openHome(email);
}
